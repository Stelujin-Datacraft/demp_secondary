...
router.get("/details", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Failed to fetch issues detail" });
  }
});
...
router.get("/detail", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Failed to fetch issue detail" });
  }
});
...
router.post("/form", async (req, res) => {
  ...
  try {
    ...
    if (valid) {
      ...
      res.json({
        status: "success",
        message: "Issue form data inserted successfully",
      });
    } else {
      res.json({ status: "failed" });
    }
    ...
    res
      .status(500)
      .json({ status: "error", message: "Failed to insert issue form data" });
  }
});
...
router.put("/post_issue_form", async (req, res) => {
  ...
  try {
    ...
    if (valid) {
      ...
      const { sector, boothNumber, issue_status, issueType, issueDescription } =
        req.body;
      console.log(
        sector,
        boothNumber,
      ...
      res
        .status(200)
        .json({ status: "success", message: "Issue updated successfully" });
    } else {
      res.json({ status: "failed" });
    }
  } catch (error) {
